...
export function ReportsExportActions({ filters, disabled }: ReportsExportActionsProps) {
  ...
  const handleExport = (format: "csv" | "xlsx" | "pdf") => {
    startTransition(async () => {
      ...
      const result = await exportReportsAction({ ...filters, page: filters.page ?? 1, perPage: filters.perPage ?? 20, period: filters.period ?? "last_30_days", report: filters.report ?? "all", format });
      if (result.ok) {
