...
import { exportAnalyticsAction } from "@/actions/analytics";
...
export function AnalyticsExportActions({ filters, disabled }: AnalyticsExportActionsProps) {
  ...
  const handleExport = (format: "csv" | "xlsx" | "pdf") => {
    startTransition(async () => {
      ...
      const result = await exportAnalyticsAction({ ...filters, page: filters.page ?? 1, perPage: filters.perPage ?? 20, period: filters.period ?? "last_30_days", format });
      if (result.ok) {
